...
def analisar_faltas_detalhado(caminho_pdf, mes):
    ...
    data = {
        ...
        'Matrícula': ['20261001', '20261002'],
        'Disciplina': ['Álgebra Linear', 'Desenvolvimento Web'],
        'Total Faltas (Mês)': [12, 8],
        'Datas das Faltas': ['05/03, 12/03, 19/03', '01/03, 08/03']
    }
    return pd.DataFrame(data)

# --- INTERFACE HTML/JS ---
# Nota: Usamos concatenação '+' no JS para evitar problemas com '${}' em strings Python
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body class="bg-slate-50 min-h-screen font-sans text-slate-900">
    <div class="max-w-5xl mx-auto py-10 px-4">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm mb-8 border border-slate-200">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                <div>
                    ...
                    <select id="mes_analise" class="w-full border-slate-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2.5 border bg-white">
        ...
        <div id="resultado" class="hidden bg-white rounded-2xl shadow-sm overflow-hidden border border-slate-200">
            <div class="p-5 border-b bg-slate-50 flex justify-between items-center">
                <h2 class="font-bold text-slate-700 text-sm uppercase tracking-widest">Alunos em padrão de evasão</h2>
    ...
    <script>
        // CONFIGURAÇÃO: Altere para a URL da sua API no Vercel
        ...
        async function processarPDFs() {
            ...
            try {
                // Limpa a URL para evitar barras duplas antes do endpoint
                ...
                alert("Não foi possível conectar à API. Verifique a URL e se o CORS está configurado no Backend.");
            } finally {
                btn.disabled = false;
        ...
        // Função corrigida para usar concatenação manual (+) em vez de ${}
        function renderizarTabela(data, mes) {
            ...
            data.forEach(function(aluno) {
                ...
                // Usando concatenação padrão para garantir compatibilidade total
